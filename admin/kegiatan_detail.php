    ...
    <div class="container mt-4">
        <div class="card shadow mb-4">
            <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
                ...
                <a href="kegiatan.php" class="btn btn-sm btn-light"><i class="bi bi-arrow-left"></i> Kembali</a>
            </div>
            <div class="card-body">
        ...
        <!-- Komentar dari Kepala -->
        <div class="card shadow mb-5">
            <div class="card-header bg-warning">
                <h6 class="mb-0 text-dark"><i class="bi bi-chat-dots"></i> Komentar Kepala</h6>
            </div>
            <div class="card-body">
                <?php
                $komentar = mysqli_query($host, "SELECT * FROM komentar_kegiatan WHERE kegiatan_id = '$id' ORDER BY id DESC");
                if (mysqli_num_rows($komentar) > 0) {
                    while ($k = mysqli_fetch_assoc($komentar)) {
                        $username = isset($k['dibuat_oleh']) ? htmlspecialchars($k['dibuat_oleh']) : 'Tidak diketahui';
                        $role = isset($k['role']) ? htmlspecialchars($k['role']) : 'Kepala';
                        $tanggal = isset($k['created_at']) ? htmlspecialchars($k['created_at']) : '-';

                        echo "<div class='mb-3 p-3 border rounded bg-light'>";
                        echo "<div class='d-flex justify-content-between'>";
                        echo "<strong class='text-primary'>" . $role . " (" . $username . ")</strong>";
                        echo "<small class='text-muted'>" . $tanggal . "</small>";
                        echo "</div>";
                        echo "<p class='mt-2 mb-0'>" . nl2br(htmlspecialchars($k['komentar'])) . "</p>";
                        echo "</div>";
                    }
                } else {
                    echo "<p class='text-muted'>Belum ada komentar dari kepala.</p>";
                }
                ?>
            </div>
        </div>
